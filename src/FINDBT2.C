...
void main(void)
{
   int result;
   char buffer[512];
   char buffer1[16] = {0x01, 0x03, 0x20, 0x04,
		       0x20, 0x00, 0x00, 0x00,
		       0x10, 0x42, 0x00, 0x00,
		       0xf6, 0x0e, 0x00, 0x00};
   int cylinder, head, sector;

   for (cylinder = 0; cylinder <= 615; cylinder++) {
      for (head = 0; head < 2; head++) {
	 for (sector = 1; sector <= 32; sector ++) {

   result = biosdisk(2, 0X81, head, cylinder, sector, 1, buffer);

   result = memcmp (buffer, buffer1, 16);

     if(!result){
     printf("Found it!  CHS = %d, %d, %d\n",cylinder, head, sector);
     break;
     }
   }
   }
